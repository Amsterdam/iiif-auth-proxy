...
def get_url_info(iiif_url, source_file):
    try:
        ...
        raise ImmediateHttpResponse(response=HttpResponse("Invalid formatted url", status=400)) from e
    return url_info
...
def parse_payload(request):
    try:
        ...
        raise ImmediateHttpResponse(response=HttpResponse("JSON invalid", status=400)) from e
...
def check_email_validity(email_address):
    is_email_regex = re.compile(r"[^@]+@[^@]+\.[^@]+")  # Just basic sanity check for a @ and a dot
